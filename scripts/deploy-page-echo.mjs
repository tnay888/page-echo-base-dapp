...
const source = await readFile(resolve(process.cwd(), "contracts/PageEcho.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "PageEcho.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["PageEcho.sol"]?.PageEcho;
...
console.log(`Deploying PageEcho from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_PAGE_ECHO_CONTRACT_ADDRESS to this contract address in Vercel.");
